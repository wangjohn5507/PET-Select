...
class PersonaGenerator(BaseGenerator):
    HumanEval_Persona_prompt = '''
    You are a programming expert, especially good at Python.
    Please complete the following task in Python:
    {prompt}
    '''
    ...
    def form_technique_prompt(self, prompt, function_name=None):
        ...
        if 'HumanEval' in self.dataset_name:
            return self.HumanEval_Persona_prompt.format(prompt=prompt)
        elif 'MBPP' in self.dataset_name:
            return self.MBPP_Persona_prompt.format(prompt=prompt, function_name=function_name)
        else:
            return self.APPS_Persona_prompt.format(prompt=prompt)
